...
export function SourceWorkbench({ documents, busy, onRead, children }: { children?: ReactNode; documents: InboxDocument[]; busy: boolean; onRead(samples: SourceSample[]): void }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/inbox/samples', { signal: controller.signal }).then(async response => {
      if (!response.ok) throw new Error('Could not load sample inputs. Reload to try again.');
      setCatalog((await response.json()).samples);
    }).catch(error => { if (!controller.signal.aborted) setError(error.message); });
    return () => controller.abort();
  }, []);
  const unread = catalog.filter(sample => !documents.some(document => document.sha256 === sample.sha256));
  ...
  const document = documents.find(d => d.sha256 === sample?.sha256);
  const facts = document?.evidence?.facts, request = document?.evidence?.request;
  return <section className={styles.workbench} aria-label="Connected sources and sample explorer">
    <div className={styles.connections}>{(['forms','email','dropbox'] as const).map(kind => <article key={kind}>
      ...
      <div><Button variant="outline" size="sm" onClick={() => setConnection(kind)}><Plug size={14} />Connect</Button><button className={styles.browse} onClick={() => { setSource(kind); setSelected(''); }}>View samples<ArrowRight size={13} /></button></div>
    </article>)}</div>
    {children}
    <div className={styles.explorerHeading}><div><h2>See what comes in. See what Sift finds.</h2><p>Sample workspace · fictional inputs; accounts are not connected.</p></div><Button disabled={busy || !unread.length} onClick={() => onRead(unread)}><Sparkles size={15} />{documents.length ? unread.length ? "Read remaining inputs" : "All sample inputs read" : "Read all sample inputs"}</Button></div>
    {error && <p role="alert">{error}</p>}
    <div className={styles.explorer}>
      <div className={styles.inputList}><div className={styles.tabs} role="group" aria-label="Sample source">{(['forms','email','dropbox','upload'] as const).map(kind => <button key={kind} aria-pressed={source === kind} aria-label={sourceNames[kind]} onClick={() => { setSource(kind); setSelected(''); }}><SourceIcon source={kind} size={21} /></button>)}</div><strong>{sourceNames[source]}</strong><small>{source === 'forms' ? 'Event reimbursement responses' : source === 'email' ? 'expenses@example.invalid' : source === 'upload' ? 'Files read in this session' : '/HackMIT / Travel receipts'}</small>
