...
export function SourceFlow({ items, reading: activeInput }: { items: SourceActivity[]; reading?: { name: string; source: string } | null }) {
  ...
  return <section className={styles.sourceIntake} data-flow-node="sources" aria-label="Sources into the audit">
    ...
    <div className={styles.sourceFlowStatus}>{reading.length ? <><LoaderCircle className={styles.spinner} />Reading {reading.length} input{reading.length === 1 ? '' : 's'}</> : <span>{items.length ? `${parsed.length} parsed · ${confirmed.length} source files linked to saved cases${failed.length ? ` · ${failed.length} need attention` : ''}` : 'Open sources to read inputs'}</span>}<small>Source workspace · activity from this browser</small></div>
  </section>;
}
