...
export function fixtureInvestigations(rows: ReviewRow[]): InvestigationRun[] {
  return [1, 2, 3, 4, 6].map(n => {
    ...
    return { run_id: fixtureId(9100 + n), claim_id: row.id, trigger: "manual", status, outcome,
      headline: `Simulated: ${headline}`, summary: `Simulated investigation. ${headline}.`,
      ...
      findings: outcome === "resolved" || outcome === "discrepancy_found" ? [{ id: fixtureId(9200 + n), check: "amount", statement: `Simulated: ${n === 2 ? "the claim is $12.00 above the receipt" : "the receipt and claimed total agree"}.`, evidence_refs: [{ kind: "receipt", id: row.receipt!.id }] }] : [],
      ...
      steps: [{ id: fixtureId(9300 + n), run_id: fixtureId(9100 + n), sequence: 1, tool: "read_receipt", status: n === 6 ? "failed" : "completed",
        started_at: started, completed_at: finished, summary: "Simulated: inspected the saved receipt.",
