-- Permit the 80-claim live demo and retain compatibility with the 14-claim local fixture.
-- Applying this migration changes only reset validation; it does not reset or seed records.
...
declare archive_id uuid; previous_knowledge bigint; next_revision bigint; seed_count integer;
begin
  perform core_lock();
  ...
  seed_count := jsonb_array_length(p_seed->'submissions');
  if jsonb_typeof(p_seed->'submissions') is distinct from 'array' or seed_count not in (14,80)
    or jsonb_typeof(p_seed->'receipts') is distinct from 'array' or jsonb_array_length(p_seed->'receipts')<>seed_count
    or jsonb_typeof(p_seed->'policies') is distinct from 'array' or jsonb_array_length(p_seed->'policies')<>5
    or jsonb_typeof(p_seed->'supporting_documents') is distinct from 'array' or jsonb_array_length(p_seed->'supporting_documents')<>(case when seed_count=80 then 20 else 8 end)
    then raise exception 'INVALID_RESET_SEED'; end if;
  if (select array_agg(x->>'id' order by x->>'id') from jsonb_array_elements(p_seed->'submissions') x)
      is distinct from (select array_agg('41000000-0000-4000-8000-'||lpad(n::text,12,'0') order by n) from generate_series(1,seed_count) n)
    or (select array_agg(x->>'id' order by x->>'id') from jsonb_array_elements(p_seed->'receipts') x)
      is distinct from (select array_agg('62000000-0000-4000-8000-'||lpad(n::text,12,'0') order by n) from generate_series(1,seed_count) n)
    ...
    or (select array_agg(x->>'id' order by x->>'id') from jsonb_array_elements(p_seed->'supporting_documents') x)
      is distinct from (select array_agg('64000000-0000-4000-8000-'||lpad(n::text,12,'0') order by n) from unnest(case when seed_count=80 then array[5,7,9,10,13,15,17,19,47,49,69,71,72,91,113,115,135,137,138,157] else array[5,7,9,10,13,15,17,19] end) n)
    then raise exception 'INVALID_RESET_SEED'; end if;
