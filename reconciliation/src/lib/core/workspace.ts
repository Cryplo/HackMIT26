...
export async function workspaceReviews(core:CoreService):Promise<ReviewsResponse> {
 ...
 return {contract_version:2,snapshot_token:snapshot.token,knowledge_revision:state.knowledge_revision??0,capabilities:{rule_learning:!!core.intelligence,extraction_retry:true,export:true,custom_checks:false,duplicate_links:true,knowledge_revisions:true},coverage:{complete:true,returned:snapshot.rows.length,total:snapshot.rows.length},submissions:rows,demo_mode:core.demoMode,
 ...
 execution:{extraction:'See each receipt extraction provenance',decisions:core.execution?.decisions||'unknown',retrieval:core.execution?.retrieval||'stored candidates',storage:core.execution?.storage||'unknown',investigation:'not enabled'}};
}
