...
def field_text(context):
    ...
    result = post_json(
        ...
        key,
        {
            ...
            # Mercury's reasoning shares the output budget and can truncate even
            # a one-word field value. This helper only needs the JSON value.
            **({"reasoning": {"enabled": False}} if model.startswith("inception/mercury") else {}),
            "messages": [
    ...
    try:
        ...
        if str(error) == "truncated":
            raise ValueError("Text helper reached its output limit; nothing typed. Choose next to retry.") from None
        raise ValueError("Text helper returned no valid field value; nothing typed. Choose next to retry.") from None
    return value, {
        "model": model,
        "latency_ms": round((time.perf_counter() - started) * 1000),
        "usage": result.get("usage", {}),
