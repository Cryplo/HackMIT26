...
from .snake import MAX_MOVES, SnakeAgent
...
def response_state():
    ...
    return {**state, "text_model": os.environ.get("TEXT_MODEL", "inception/mercury-2.5"),
            "max_steps": MAX_MOVES if state.get("scenario") == "snake" else MAX_STEPS}
...
def command(name, body):
    ...
    if name == "reset":
        ...
        if scenario not in {"travel", "research", "flights", "snake"}:
            raise ValueError("Unknown demo scenario")
        goal = body.get("goal", "").strip()
        if not goal or len(goal) > 2000:
            raise ValueError("Enter 1–2,000 characters")
        if not os.environ.get("AI_GATEWAY_API_KEY", "").strip():
            raise ValueError("Set AI_GATEWAY_API_KEY in .env before starting a task")
        close_browser()
        if scenario == "snake":
            AGENT = SnakeAgent(goal)
    ...
    def do_POST(self):
        ...
        if self.headers.get("X-Demo-Token") != TOKEN:
            return self.send(403, json.dumps({
                ...
                "error": "Dashboard session expired after a server restart. Refresh this page and try again.",
            }))
