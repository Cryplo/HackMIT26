-- Optional synthetic showcase reset; additive to platform v4. Applying this migration resets nothing.
do $$ begin if core_platform_version() <> 4 then raise exception 'Expected platform version 4'; end if; end $$;
create table public.demo_reset_archives (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
  snapshot jsonb not null
);
alter table public.demo_reset_archives enable row level security;
revoke all on public.demo_reset_archives from public,anon,authenticated,service_role;
grant select on public.demo_reset_archives to service_role;
...
begin
  ...
  if (select array_agg(x->>'id' order by x->>'id') from jsonb_array_elements(p_seed->'submissions') x)
    ...
    or (select array_agg(x->>'id' order by x->>'id') from jsonb_array_elements(p_seed->'receipts') x)
      is distinct from (select array_agg('42000000-0000-4000-8000-'||lpad(n::text,12,'0') order by n) from generate_series(1,14) n)
    ...
    or (select array_agg(x->>'id' order by x->>'id') from jsonb_array_elements(p_seed->'supporting_documents') x)
      is distinct from (select array_agg('44000000-0000-4000-8000-'||lpad(n::text,12,'0') order by n) from unnest(array[5,7,9,10,13,15,17,19]) n)
    then raise exception 'INVALID_RESET_SEED'; end if;
  if exists(select 1 from jsonb_array_elements(p_seed->'submissions') s where
    ...
    or exists(select 1 from jsonb_array_elements(p_seed->'receipts') r where
      r->>'submission_id' is distinct from replace(r->>'id','42000000-','41000000-')
