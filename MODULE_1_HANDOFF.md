...
3. Module 2 migration/seeded files using other storage paths will not be served by this endpoint until the integration explicitly aligns synthetic namespace conventions. No generic storage-path endpoint exists.
4. Local intake demo uses per-record JSON and private binary files, independent of Module 2's demo adapter. Therefore local submissions do not automatically appear in Module 2/3 demo views. Live Supabase is the shared integration path. No changes were made to other modules to hide this boundary.
