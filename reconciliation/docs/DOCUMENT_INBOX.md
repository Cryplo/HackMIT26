...
- Supabase imports require migration `202609210013_inbox_text_evidence.sql`, which permits the validated CSV/TXT/EML types in supporting records and the default private `receipts` bucket. If `SUPABASE_RECEIPTS_BUCKET` names another bucket, configure its allowed MIME types to the same six formats. The receipts table itself has no MIME constraint. This migration does not reset claims or change bucket privacy.
- Resetting claims retains separate inbox staging. Retrying a saved import whose claim or receipt was removed returns `stale_import`; stale source batches show reset guidance instead of successful completion. Reload and upload originals as a new manual batch, or restart the isolated source-demo launcher.
- Merge verification: six focused offline inbox tests passed on Node 24.11.1, including stale confirmation/source-batch checks and a PGlite migration check preserving existing PDF evidence, admitting the six validated MIME types, rejecting HTML, and retaining bucket privacy. The migration was not applied to a live database during these checks.
...
The sample spreadsheet contains 21 fictional responses with varied dates, origins, categories, and amounts. Choose a response in the row selector, then **Parse selected row**; **Download selected row** exports that request with its header. Requested amounts remain separate from receipt totals. The original full spreadsheet is available for viewing/download but is rejected by intake as a multi-request file. **Read all sample inputs** and the optional source audit use the derived `Ava-response.csv` single-row input, preserving the existing linked receipt/booking example. Other responses can be parsed individually; they do not acquire fabricated receipts or links.
