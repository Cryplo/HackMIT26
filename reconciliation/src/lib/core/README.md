...
Entry point: `runtime.ts`; orchestration: `service.ts`; public v2 types: `../review-contracts.ts`. Start with [current project context](../../../../docs/PROJECT_CONTEXT.md). The original investigation contracts in `../../../../docs/archive/2026-09-20-review-workflow/00-contracts.md` are historical design background. This remains an unauthenticated synthetic-data-only demo. Keep originals private and credentials on the server.
...
## Integrated automation and feedback learning

`automation.ts` approves eligible current clean claims under policy caps unless automation is disabled. This is separate from human corrections; flags and processing failures do not become automatic rejections. Useful available supporting evidence can trigger bounded investigation and reassessment. Saved human decisions survive rechecks.

`feedback-learning-state.ts` saves the human correction and learning job atomically. The request-lifecycle worker in `feedback-learning.ts` classifies an internal reason, derives a narrow hotel booking-reference identity check, runs the twelve-case procedure suite, and activates only current passing proof. It then rechecks undecided claims, using each claim’s own documents. This is separate from the ten-case alias lifecycle described above and the advanced manual investigation workflow. See [Review learning](../../../docs/REVIEW_LEARNING.md) for scope, privacy, retry, and limitations. Request-lifecycle workers are not a durable external task queue.
...
Current investigation-platform verification on Node 24.11.1: 130/130 application tests (including 7 SQL tests), 25/25 intelligence tests, 23/23 investigation-pack checks, 25/25 browser tests, TypeScript checking, and production build passed against integrated upstream `e9d5b97`. See [project context](../../../../docs/PROJECT_CONTEXT.md) for current verification and deployment notes. PGlite exercises local SQL transactions with test Supabase roles/storage; it does not prove remote deployment.
...
Automatic notices also accompany eligible policy approvals and decision-only routes. Notices remain held until explicit batch confirmation through **Send all notifications**. In live email mode the dispatcher then attempts eligible released notices; the separate `npm run email:worker` process acquires durable leases and sends frozen payloads with stable Resend keys. It records provider acceptance without claiming inbox delivery. A saved decision survives delivery failure; retries operate on the same message and never create another correction. Later decisions cancel obsolete unsent messages and respect active send leases. Confirmation request IDs let clients recover interrupted responses without creating another decision.

Migration 004 introduced application readiness version 4. Current deployments also require the subsequent additive migrations, including automatic notices (006) and feedback learning (010). See the app README for the ordered list; version 4 alone does not prove all additive migrations are installed. See [Resend setup and rollout](../../../docs/resend-setup.md) for configuration and worker operation. Request-information/resumption and webhook delivery tracking remain later phases.
