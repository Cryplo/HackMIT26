...
test("preview learning is separate, versioned, test-gated, and stale until an explicit recheck", async () => {
  ...
  assert.equal(tested.mode, "simulated");
  assert.equal(tested.passed, true);
  await assert.rejects(client.activateRule(draft.id, { expected_rule_version: 1 }), code("STALE_RULE"));
  const active = await client.activateRule(draft.id, { expected_rule_version: tested.rule_version });
...
test("proposal/test rejection, stale test gate, retry extraction, and read-only search stay honest", async () => {
  ...
  await assert.rejects(client.activateRule(failed.rule_id, { expected_rule_version: failed.rule_version }), code("TEST_FAILED"));
  ...
  const active = await client.activateRule(passed.rule_id, { expected_rule_version: passed.rule_version });
  await client.disableRule(active.rule.id, { expected_rule_version: active.rule.version });
  await client.disableRule(failed.rule_id, { expected_rule_version: failed.rule_version });
  const original = (await client.getReviews()).submissions[5];
  const url = client.receiptUrl(original);
  const retried = await client.retryExtraction(fixtureId(6), 1);
  assert.equal(retried.row.id, original.id);
  assert.equal(retried.row.receipt?.id, original.receipt?.id);
  assert.equal(retried.row.receipt?.sha256, original.receipt?.sha256);
  assert.equal(client.receiptUrl(retried.row), url);
