...
```ts
await backfillReceiptHashes(core, getStore());
```

Review the target database and private bucket before shared invocation. This has **not** been run against shared Supabase. It reads original bytes, computes SHA-256, skips existing hashes, leaves absent originals null, and reports `hashed`/`unavailable`. It does not alter bytes or parsed fields and makes no model calls. New evidence can invalidate assessments and dependent rules, so refresh and recheck afterward. Do not generate replacement originals to fill missing hashes.
...
Outstanding: shared migration and reviewed hash backfill; authorized paid live verification budget; integration-owner full build/browser checks; Devin's independent held-out evaluation. PR #5 remains held for integration-owner review; this platform does not require its broader changes. `scripts/check-jev.ts` still uses legacy `vendor_alias` and belongs to another owner: update it to the reviewed rule workflow before use. No live provider calls or credentials were used to capture these examples.
