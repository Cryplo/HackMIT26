...
export async function workspaceReviews(core:CoreService):Promise<ReviewsResponse> {
 ...
 return {contract_version:2,snapshot_token:snapshot.token,knowledge_revision:state.knowledge_revision??0,capabilities:{demo_reset:demoReset,automatic_processing:core.automationEnabled,automatic_decision_emails:email.decision_email_drafts,decision_email_drafts:email.decision_email_drafts,decision_emails:email.decision_emails,email_mode:email.email_mode,email_error:email.error,supporting_documents:true,investigations:core.investigationMode!=='disabled',resolution_procedures:!!core.intelligence?.build_procedure_suite&&!!core.intelligence?.evaluate_procedure,rule_learning:!!core.intelligence,extraction_retry:true,export:true,custom_checks:true,duplicate_links:true,knowledge_revisions:true},coverage:{complete:true,returned:snapshot.rows.length,total:snapshot.rows.length},submissions:rows,demo_mode:core.demoMode,
