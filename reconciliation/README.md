...
Every completed run stores a reviewer-facing justification inside the `overall_status` decision, and `POST /api/justifications` with `{"submission_id": "<uuid>"}` regenerates one on demand from the latest completed run. The narrative only restates checks that already ran: it cannot change a verdict, a status, or a stored decision. Demo and simulated modes emit a deterministic template. Set `RECONCILIATION_JUSTIFICATION_MODE=live` with `OPENAI_API_KEY` (model override: `JUSTIFICATION_MODEL`, default `gpt-4.1-mini`) outside simulated mode for OpenAI narratives, which incur provider usage and are logged in `model_calls`. If OpenAI refuses, times out, or returns an unusable narrative, the run keeps its outcome and falls back to the deterministic summary with an `error` code attached.
