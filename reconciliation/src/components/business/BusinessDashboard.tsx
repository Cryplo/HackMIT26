...
export default function BusinessDashboard({ preview = false }: { preview?: boolean }) {
  ...
  return (
    <AppShell view={view} onViewChange={setView} preview={preview}>
      ...
      {error && <div role="alert" className={`${styles.error} motion-enter`}><AlertCircle aria-hidden="true" /><div><strong>{error}</strong><p>{data ? "Showing the last successful snapshot. Your selection and open claim are preserved." : "Connect the v2 review API, or explore the labeled synthetic preview."}</p><div className={styles.inlineActions}><Button variant="outline" onClick={() => void refresh().catch(() => {})}>Retry connection</Button>{!preview && <Link href="/business-demo?preview=1">Open synthetic preview <ArrowUpRight aria-hidden="true" /></Link>}</div></div></div>}
