...
export function ChecksPanel({ client, knowledgeRevision, capabilities, simulatedEnvironment, onChanged }: ChecksPanelProps) {
  ...
  return <section className="space-y-6" aria-label="Checks">
    ...
    {client.mode === "preview" && <div className="flex items-start gap-2 text-sm text-muted-foreground"><FlaskConical className="mt-0.5 size-4 shrink-0" aria-hidden="true" /><p>Preview — synthetic data. Custom checks stay “needs review” in the simulated evaluation; no live provider judges them.</p></div>}
