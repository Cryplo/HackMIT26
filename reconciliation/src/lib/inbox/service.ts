...
export async function confirmImport(body: unknown, dependencies: { core: CoreService; intake: IntakeStore; originals: SupportingOriginals; dir?: string }): Promise<ImportResult> {
  ...
  try {
    ...
    if (prior) {
      if (prior.status === 'saved' && prior.fingerprint === fingerprint && prior.result) {
        const state = await dependencies.core.store.snapshot();
        if (!state.submissions.some(claim => claim.id === prior.result!.submission_id)
          || !state.receipts.some(receipt => receipt.id === prior.result!.receipt_id && receipt.submission_id === prior.result!.submission_id)) {
          throw new IntakeError('stale_import', 'This saved import is no longer in the workspace, possibly after a reset. Reload the workspace and upload the originals as a new batch; for the source demo, restart the isolated launcher.', 409);
        }
        return prior.result;
      }
