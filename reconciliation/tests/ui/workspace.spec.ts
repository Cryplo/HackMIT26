...
test("insight count and cents drill into exact IDs, recompute on refresh and clear before AI search", async ({ page }) => {
  ...
  await page.getByLabel("What are you looking for?").fill("hotel receipts");
  expect(searches).toBe(0);
  await page.getByRole("button", { name: "Search claims", exact: true }).click();
...
test("semantic search is explicit, ignores delayed responses after filters change and opens current rows", async ({ page }) => {
  ...
  await page.goto("/business-demo");
  await page.getByRole("button", { name: "Ask about claims", exact: true }).click();
  await page.getByLabel("What are you looking for?").fill("hotel");
  ...
  release();
  await expect(page.getByRole("button", { name: "Search claims", exact: true })).toBeEnabled();
...
test("export downloads only explicit selection and captured token, revokes URL, and creates no file on conflict", async ({ page }) => {
  ...
  await page.goto("/business-demo");
  await page.getByRole("button", { name: "Ask about claims", exact: true }).click();
  await page.getByLabel("What are you looking for?").fill("claims");
  await page.getByRole("button", { name: "Search claims", exact: true }).click();
