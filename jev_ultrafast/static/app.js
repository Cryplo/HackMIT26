...
let state = null,
  ...
  automatic = false;
const goals = {
  snake: 'Collect 3 apples in Google Snake without hitting a wall or the snake. The game pauses between moves.',
...
async function call(name, body = {}) {
  ...
  if (response.status === 403 && data.code === "session_expired") {
    // This request was rejected before execution. Reload the token and state;
    // leave the next browser action to the user instead of replaying a command.
...
function render() {
  if (!state) return;
  $("helper").textContent = state.scenario === "snake"
    ? "Jev chooses every direction · canvas reader · game clock pauses between moves · target: 3 apples"
  ...
  if (state.scenario === "snake") {
    labels.ready = `Score ${state.snake.score} / 3 · game paused · ready for Jev's next move`;
    labels.done = "3 apples collected · verified from the game score";
    labels.blocked = "Snake stopped · no safe move, move limit, or board did not advance as expected";
  }
  ...
  $("action-count").textContent = state.scenario === "snake"
    ? `Score ${state.snake.score} / 3` : `${state.elements.length} elements`;
  ...
  $("choices-label").textContent = state.scenario === "snake" ? "Observed board" : "Indexed elements";
  if (state.scenario === "snake") $("ranking-note").textContent = "17 × 15 · canvas pixels";
  ...
  if (state.scenario === "snake") {
    $("choices").innerHTML = `<p class="muted">Head: ${escape(JSON.stringify(state.snake.head))}<br>` +
      `Apples: ${escape(JSON.stringify(state.snake.apples))}<br>` +
      `Clock paused between moves. Jev chooses each arrow direction.</p>`;
  }
...
fetch("/api/state")
  ...
  .then((s) => {
    state = s;
    if (s.scenario && goals[s.scenario]) {
      $("scenario").value = s.scenario;
      $("goal").value = s.goal || goals[s.scenario];
    }
    render();
  })
  .catch(() => {
