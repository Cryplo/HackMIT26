...
test('real private upload → match → confirm → review preserves discrepancy, originals, and retry identity', async () => {
  ...
  try {
    ...
    const resetCore = new CoreService(new FileStore(path.join(dir, 'reset-claims')), new DatabaseRetrieval(), new SimulatedJev(), true);
    await assert.rejects(confirmImport(body, { ...dependencies, core: resetCore }), { code: 'stale_import', status: 409 });
    assert.deepEqual(await confirmImport(body, dependencies), result, 'stale lookup must preserve the saved confirmation');
...
test('source audit reads mixed originals once, queues complete requests, and holds uncertain evidence', async () => {
  ...
  try {
    ...
    let batch = await readSourceAudit(deps.dir, core);
    ...
    const resolved = await readSourceAudit(deps.dir, core);
    assert.equal(resolved.imports.length, 3, 'manual resolutions survive source-page reloads');
    assert.equal(resolved.held.length, 2);
    const resetCore = new CoreService(new FileStore(path.join(dir, 'reset-claims')), new DatabaseRetrieval(), new SimulatedJev(), true);
    const stale = await readSourceAudit(deps.dir, resetCore);
    assert.equal(stale.phase, 'failed');
    assert.match(stale.error, /reset.*Restart the isolated launcher/);
    assert.equal(stale.documents.length, resolved.documents.length, 'reset guidance preserves source evidence');
    assert.deepEqual(await advanceSourceAudit(request(Buffer.from('unused')), 'demo', { ...deps, core: resetCore }), stale);
  } finally {
...
test('inbox migration permits validated text evidence in SQL and the private storage bucket', async () => {
  const db = new PGlite();
  const migration = async (name: string) => db.exec((await readFile(new URL(`../../../supabase/migrations/${name}.sql`, import.meta.url), 'utf8')).replace('create extension if not exists pgcrypto;', ''));
  try {
    ...
    for (const name of ['202609190001_reimbursement_core', '202609200002_platform', '202609200003_investigations']) await migration(name);
    const claimId = randomUUID();
    await db.query("insert into submissions(id,attendee_name,email,amount_requested_minor,category,origin_location) values($1,'Inbox Demo','inbox@example.invalid',100,'bus','Boston')", [claimId]);
    async function insertDocument(fileType: string, index: number) {
      const id = randomUUID();
      await db.query("insert into supporting_documents(id,claim_id,kind,storage_path,file_type,sha256,created_at,extraction_status) values($1,$2,'other',$3,$4,$5,now(),'pending')",
        [id, claimId, `synthetic/${claimId}/supporting/${id}`, fileType, String(index).padStart(64, '0')]);
    }
    await assert.rejects(insertDocument('text/csv', 0), /supporting_documents_file_type_check/);
    await insertDocument('application/pdf', 1);
    await migration('202609210013_inbox_text_evidence');
    const formats = ['application/pdf', 'image/png', 'image/jpeg', 'text/csv', 'text/plain', 'message/rfc822'];
    for (const [index, type] of formats.entries()) await insertDocument(type, index + 2);
    await assert.rejects(insertDocument('text/html', 9), /supporting_documents_file_type_check/);
    assert.equal((await db.query<{ count: number }>('select count(*)::int as count from supporting_documents')).rows[0].count, 7);
    const bucket = (await db.query<{ public: boolean; allowed_mime_types: string[] }>("select public,allowed_mime_types from storage.buckets where id='receipts'")).rows[0];
    assert.equal(bucket.public, false);
    assert.deepEqual(bucket.allowed_mime_types, formats);
    await db.query("insert into receipts(submission_id,storage_path,file_type) values($1,'synthetic/test','text/plain')", [claimId]);
