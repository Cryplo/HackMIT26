-- Prepare 70 checked claims and leave the original ten audit cases unchecked.
-- Preserve archive, stale-snapshot, namespace, processing and original-evidence guards.
...
declare result jsonb; raw_seed jsonb; custom_archive jsonb; revision bigint; knowledge bigint; prepared boolean;
  unchecked integer[] := array[1,2,5,6,7,9,10,12,13,14];
begin
  perform core_lock();
  ...
  if prepared then
    if jsonb_typeof(p_seed) is distinct from 'object'
      ...
      or jsonb_typeof(p_seed->'runs') is distinct from 'array' or jsonb_array_length(p_seed->'runs') <> 70
      or jsonb_typeof(p_seed->'decisions') is distinct from 'array' or jsonb_array_length(p_seed->'decisions') <> 637
      or jsonb_typeof(p_seed->'corrections') is distinct from 'array' or jsonb_array_length(p_seed->'corrections') <> 7
  ...
  if prepared then
    if (select array_agg(r->>'id' order by r->>'id') from jsonb_array_elements(p_seed->'runs') r)
      ...
      is distinct from array['73000000-0000-4000-8000-000000000015','73000000-0000-4000-8000-000000000030','73000000-0000-4000-8000-000000000035','73000000-0000-4000-8000-000000000037','73000000-0000-4000-8000-000000000044','73000000-0000-4000-8000-000000000059','73000000-0000-4000-8000-000000000068']
      then raise exception 'INVALID_RESET_SEED'; end if;
    ...
    if exists(select 1 from jsonb_array_elements(p_seed->'submissions') s where
      ...
      or (select count(*) from jsonb_array_elements(p_seed->'decisions') d where d->>'check_method'='human') <> 7
