...
## Merchant-corroboration trade-off surfaced by this run
...
The merchant instruction text changed in commit `e9d5b97` ("Add bounded investigation planner and cancellable Jev adapter"): it now asks Jev to use "linked booking evidence if supplied" and says "missing or conflicting corroboration ... require unknown". Plain receipts carry no booking corroboration, so Jev abstains. A four-case control run of the benchmark checked out at `9f3d593` (pre-`e9d5b97` production) against the same live providers matched 3/4 of the familiar cases, so this is not a provider-side change.

This is a deliberate product choice rather than a bug: the showcase seed (the 14 claims in Supabase) is built around a billing descriptor, "Harbor Reservations", that must not be approved as a hotel without a matching booking confirmation, and the [showcase run](../2026-09-20-showcase/README.md) shows the direct-AI baseline approving all four such claims while Sift holds them. The cost is that this synthetic dataset's generic single-receipt merchants, which have no booking documents, now abstain too. Nothing under `src/` was edited for this benchmark. Until the prompt distinguishes descriptor-style merchants from plain named merchants (or the dataset carries corroborating documents), Sift's valid-match yield on this dataset and the equivalent-quality savings headline cannot be presented.
