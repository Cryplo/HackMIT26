...
export function validateAnswers(raw: unknown): Record<SemanticField, Answer> {
  if (!isObject(raw) || Reflect.ownKeys(raw).length !== 3 || Object.keys(questions).some(key => !Object.hasOwn(raw, key))) throw new CoreError('JEV_INVALID', 'Jev returned invalid answers.', 503);
...
export class LiveJev implements Jev {
  ...
  async evaluate(state: SemanticState, runId: string, log: (call: ModelCall) => Promise<void>, signal?: AbortSignal): Promise<Evaluation> {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        const body: unknown = await res.json(); signal?.throwIfAborted(); if (!isObject(body)) throw new CoreError('JEV_INVALID', 'Invalid Jev response.', 503); raw = body;
