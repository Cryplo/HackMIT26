...
With migration `202609210014_live_demo_baseline.sql`, live **Reset demo** restores **60 checked claims and 20 unchecked claims**. The 60 checked claims have explicitly **prepared demo history**, calculated offline with the simulated fixture evaluator: 52 approved, four authored rejection examples, and four inconclusive claims awaiting review. This is not a claim that live Jev or a human reviewer produced those historical outcomes. Every prepared run and decision carries `demo_baseline: true` and provenance; no model usage, investigations, notices, or feedback-learning jobs are invented or triggered during reset. The original 80 receipt PDFs and 20 supporting PDFs stay unchanged. The separate local fourteen-claim simulation is unchanged.

The remaining 20 use ordinary live checking after **Start audit**. Their seed numbers are UUID suffixes on `41000000-0000-4000-8000-`:

| Seed numbers | Scenario |
| --- | --- |
| 1, 2, 13, 14, 16–22 | Eleven clean flight, train, and bus candidates |
| 9, 15 | Requested amount differs from the receipt |
| 10, 30, 44 | Hotel, train, and bus policy caps exceeded |
...
The offline evaluator gives eleven passes, six confirmed failures, and three inconclusive outcomes for these twenty; only Morgan and Riley qualify for automatic investigation. Live outcomes remain model-dependent. The four prepared rejections are claims 35, 37, 59, and 68; their reasons begin **Prepared demo decision:**. Claims 36, 47, 69, and 80 are already checked but still need review.
...
This verifies the counts, unchanged PDFs, twenty-claim case mix, two investigation candidates, SQL round-trip approval markers, advancing revisions, custom-check archival, and atomic rollback. Applying the migration alone does not reset live data.


The showcase on port 3002 is intentionally simulated. The live app at `http://127.0.0.1:3000` uses Supabase persistence, Azure/OpenAI extraction, Jev assessment, and an Azure investigator with persisted read-tool calls. Intake, extraction, assessment, and investigation are enabled in the live environment; automation uses the policy caps. Applicant email remains template/preview only; decision and policy-approval notices are created automatically.
...
Automatic notice support uses additive migration `202609200006_automatic_notices.sql`. It was applied to the live demo database on September 20, 2026. Applying it sends no emails and does not backfill historical notices. Human decision plus immutable notice use the existing atomic confirmation transaction; requests and unchanged policy approvals are deduplicated. Live email mode attempts delivery immediately and retains the outbox worker for durable retries (`npm run email:worker`). Provider acceptance does not prove inbox delivery.
