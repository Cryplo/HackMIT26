...
import { checkConfiguration, mutateCustomCheck, type CheckCommand, type StoredCustomCheck } from './custom-checks';
export interface Snapshot { claim_messages?:ClaimMessage[]; message_delivery_events?:MessageDeliveryEvent[]; supporting_documents?:StoredDocument[]; investigations?:InvestigationRun[]; procedures?:StoredProcedure[]; procedure_history?:StoredProcedure[]; procedure_tests?:ProcedureAttempt[]; knowledge_revision?: number; rules?: StoredRule[]; rule_history?: StoredRule[]; rule_tests?: RuleAttempt[]; custom_checks?: StoredCustomCheck[]; custom_check_history?: StoredCustomCheck[]; extraction_history?: Receipt[]; submissions: Submission[]; receipts: Receipt[]; policies: PolicyRule[]; decisions: Decision[]; corrections: Correction[]; runs: ReconciliationRun[] }
export interface Store { feedbackLearning(command:FeedbackCommand):Promise<FeedbackResult>; messages(command:MessageCommand):Promise<MessageResult>; supporting(command:SupportingCommand):Promise<{document:StoredDocument;lease:string}>; investigation(command:InvestigationCommand):Promise<InvestigationRun>; procedure(command:ProcedureCommand):Promise<{procedure:StoredProcedure;knowledge_revision:number}>; customCheck(command:CheckCommand):Promise<{check:StoredCustomCheck;knowledge_revision:number}>; snapshot(): Promise<Snapshot>; begin(id: string): Promise<string>; finish(run: string, ds: Decision[], status: SubmissionStatus): Promise<void>; fail(run: string, message: string): Promise<void>; correct(input: CorrectionInput): Promise<{ correction_id: string; status: SubmissionStatus }>; usage(call: ModelCall): Promise<void>; rule(command: RuleCommand): Promise<{rule:StoredRule;knowledge_revision:number}>; beginExtraction(id:string, revision:number):Promise<string>; finishExtraction(lease:string, receipt:Receipt):Promise<void>; receiptHash(id:string, hash:string):Promise<void> }
...
export class MemoryStore implements Store {
  ...
  constructor(public state: Snapshot) {
    state.claim_messages??=[];state.message_delivery_events??=[];state.supporting_documents??=[];state.investigations??=[];state.procedures??=[];state.procedure_history??=[];state.procedure_tests??=[];state.knowledge_revision??=0;state.rules??=[];state.rule_history??=[];state.rule_tests??=[];state.custom_checks??=[];state.custom_check_history??=[];state.extraction_history??=[];
  ...
  private evidence(){return {supporting_documents:this.state.supporting_documents,active_procedures:this.state.procedures?.filter(p=>p.state==='active'),active_aliases:activeAliases(this.state),check_configuration:checkConfiguration(this.state),receipts:this.state.receipts,policies:this.state.policies,claims:this.state.submissions.map(({id,attendee_name,amount_requested_minor,currency,category,submitted_at})=>({id,attendee_name,amount_requested_minor,currency,category,submitted_at}))};}
  async rule(command:RuleCommand){return mutateRule(this.state,command);}
  async customCheck(command:CheckCommand){return mutateCustomCheck(this.state,command);}
...
export class SupabaseStore implements Store {
  ...
  customCheck(command:CheckCommand){return this.request('rpc/core_custom_check',{p_input:command});}
