...
export async function mutationBody(request: Request): Promise<unknown> {
  ...
  if (Number(request.headers.get('content-length') || 0) > 65536) throw new CoreError('BODY_TOO_LARGE', 'Request body exceeds 64 KiB.', 413);
  const reader = request.body?.getReader(); if (!reader) throw new CoreError('INVALID_JSON', 'JSON body required.');
  let size = 0; const chunks: Uint8Array[] = [];
  while (true) { const { done, value } = await reader.read(); if (done) break; size += value.length; if (size > 65536) { await reader.cancel(); throw new CoreError('BODY_TOO_LARGE', 'Request body exceeds 64 KiB.', 413); } chunks.push(value); }
