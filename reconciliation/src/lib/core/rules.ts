...
export async function proposeRule(core:CoreService,raw:unknown){port(core);const result=await core.rule({action:'propose',...parsed(proposal,raw)});return {...result,rule:publicRule(result.rule)};}
export async function changeRule(core:CoreService,id:string,action:'test'|'activate'|'disable',raw:unknown,signal:AbortSignal){
 ...
 if(action==='disable'){const result=await core.rule({action,id,expected_rule_version});return {...result,rule:publicRule(result.rule)};}
 ...
 if(action==='activate'){const result=await core.rule({action,id,expected_rule_version,...binding});return {...result,rule:publicRule(result.rule)};}
 signal.throwIfAborted();
 const started=await core.rule({action:'test',id,expected_rule_version,...binding});
 ...
 try{
  ...
  const saved=await core.rule({action:'tested',id,attempt_id,report,error:null,observations});
  ...
  await core.rule({action:'tested',id,attempt_id,report:null,error:error instanceof CoreError?error.code:signal.aborted?'ABORTED':'RULE_TEST_FAILED',observations});
