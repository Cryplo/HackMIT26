...
- Code enforces amounts, dates, and caps. Jev judges merchant/category compatibility, attendee identity, and duplicate evidence. Reviewers can add their own Jev questions from the workspace Checks page (`/api/checks`); custom checks return calibrated pass/fail/needs-review verdicts, join the required set for `matched`, and can never override a failed financial or duplicate check. Changing check configuration marks prior assessments for recheck. Code combines independent checks. Missing fields and uncertainty require review. Thresholds are conservative starting values, not calibrated on representative data.
