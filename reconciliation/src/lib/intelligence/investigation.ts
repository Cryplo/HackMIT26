...
const invalid = (message:string) => new CoreError('INVALID_PROVIDER_OUTPUT',message,503);
...
/** Low-level planner. B's adapter must bind execute to its persisted, cancellable read tools.
 * Until B delivers the shared types/config, this is deliberately not exposed by IntelligencePort.
...
export async function runInvestigationPlanner(input: InvestigationInput, execute:(tool:Tool,signal:AbortSignal)=>Promise<unknown>, options:ProviderOptions, config:ResponsesConfig, transport:typeof fetch=fetch) {
  ...
  const observed=new Map<string,z.infer<typeof reference>>();
  ...
  const remember=(tool:Tool,value:unknown)=>{
    ...
    const add=(kind:z.infer<typeof reference>['kind'],id:unknown)=>{if(!isUUID(id))throw invalid('Tool returned an invalid evidence identity.');refs.push({kind,id});};
    if(tool==='read_receipt'&&value!==null){
      if(!isObject(value)||value.submission_id!==input.submission.id)throw invalid('Receipt belongs to a different claim.');
      if(value.raw_extracted_text!==null&&typeof value.raw_extracted_text!=='string')throw invalid('Invalid stored receipt text.');
      ...
      if(!Array.isArray(value))throw invalid('Read tool did not return a record list.');
      ...
      for(const record of value){
        if(!isObject(record))throw invalid('Invalid evidence record.');
        if(tool==='read_supporting_documents'){
          if(record.claim_id!==input.submission.id)throw invalid('Supporting document belongs to a different claim.');
          if(record.extracted_text!==null&&typeof record.extracted_text!=='string')throw invalid('Invalid supporting text.');
          ...
          if(!isObject(record.submission))throw invalid('Invalid related claim.');add('claim',record.submission.id);
          if(record.receipt!==null){if(!isObject(record.receipt)||record.receipt.submission_id!==record.submission.id)throw invalid('Related receipt ownership mismatch.');add('receipt',record.receipt.id);}
  ...
  for(let round=0;round<3;round++){
    ...
    try{
      const response=await transport(config.url,{method:'POST',headers:responsesHeaders(config),signal,body:JSON.stringify({model:config.model,store:false,max_output_tokens:4000,instructions:`${instructions} This is request ${round+1} of 3; ${6-toolCount} tool executions remain.`,input:history,tools,tool_choice:round===2?'none':'auto',text:{format:{type:'json_schema',name:'investigation_findings',strict:true,schema:z.toJSONSchema(finalSchema)}}})});
      signal.throwIfAborted();
      ...
      if(!isObject(body))throw invalid('Invalid investigation response.');raw=body;
    ...
    signal.throwIfAborted();
    if(!raw||raw.status!=='completed'||!Array.isArray(raw.output))throw invalid('Investigation response was incomplete.');
    const calls:{name:Tool;call_id:string}[]=[];const messages:string[]=[];const idsThisRound=new Set<string>();
    for(const item of raw.output){
      if(!isObject(item))throw invalid('Invalid response output.');
      if(item.type==='reasoning')continue; // Opaque protocol data, never persisted as public steps.
      if(item.type==='function_call'){
        if(!investigationToolNames.includes(item.name as Tool)||typeof item.call_id!=='string'||!item.call_id.trim()||callIds.has(item.call_id)||idsThisRound.has(item.call_id)||typeof item.arguments!=='string')throw invalid('Unapproved tool or invalid/reused tool call identity.');
        let args:unknown;try{args=JSON.parse(item.arguments);}catch{throw invalid('Malformed tool arguments.');}
        if(!isObject(args)||Object.keys(args).length)throw invalid('Read tools take no arguments.');
        idsThisRound.add(item.call_id);calls.push({name:item.name as Tool,call_id:item.call_id});
      }else if(item.type==='message'){
        if(item.role!=='assistant'||item.status!=='completed'||!Array.isArray(item.content))throw invalid('Invalid final message.');
        for(const content of item.content){if(!isObject(content)||content.type!=='output_text'||typeof content.text!=='string')throw invalid('Refused or invalid final content.');messages.push(content.text);}
      }else throw invalid('Unsupported investigation output item.');
    }
    if(calls.length){
      if(messages.length)throw invalid('Mixed tool calls and final findings.');
    ...
    if(messages.length!==1)throw invalid('A single structured final response is required.');
    let value:unknown;try{value=JSON.parse(messages[0]);}catch{throw invalid('Malformed structured findings.');}
    const parsed=finalSchema.safeParse(value);if(!parsed.success)throw invalid('Invalid structured findings.');
    const result=parsed.data;
    if(!result.findings.length&&!result.unresolved_question)throw invalid('Empty findings require a meaningful unresolved question.');
    if(result.next_action==='request_document'&&!result.unresolved_question)throw invalid('A document request requires an unresolved question.');
    for(const ref of [...result.findings.flatMap(f=>f.evidence_refs),...(result.proposed_learning?.source_evidence_refs??[])])if(!observed.has(`${ref.kind}:${ref.id}`))throw invalid('Finding cites unobserved or foreign evidence.');
    if(result.proposed_learning){
      if(result.proposed_learning.required_evidence.join(',')!=='receipt,booking_confirmation')throw invalid('Invalid required procedure evidence.');
      const kinds=new Set(result.proposed_learning.source_evidence_refs.map(r=>r.kind));
      if(!kinds.has('receipt')||!kinds.has('supporting_document')||input.submission.category!=='hotel'||input.submission.currency!=='USD')throw invalid('Procedure proposal lacks required scoped evidence.');
    }
