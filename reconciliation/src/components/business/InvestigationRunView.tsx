...
export function InvestigationRunView({ run, row, rows, documents, client, knowledgeRevision, onOpenClaim, showClaimLink = true }: {
  ...
  const outcome = run.status === "running" ? "Checking evidence" : run.status === "failed" ? "Needs review" : !completed ? statusLabel(run.status) : ready ? "Checks passed" : run.outcome === "needs_human" ? "Needs your input" : run.outcome === "discrepancy_found" ? "Discrepancy found" : run.outcome === "resolved" ? "Question resolved" : "Result unavailable";
  ...
  return <section aria-label="Investigation details" className={styles.runView}>
    <div className={styles.conclusion} data-failed={run.status === "failed" || undefined} data-warning={exception || undefined} role="status">
      ...
      {run.status === "failed" ? <><p>Automatic investigation could not finish. Review the saved evidence.</p><details className={styles.disclosure}><summary>Technical details</summary><p>Investigation failed. {failure.message}</p>{failure.detail && <p><code>{failure.detail}</code></p>}</details></> : run.status === "superseded" ? <p>The evidence changed during this check. Review the latest claim result.</p> : current && row ? <p>{claimReason(row)}</p> : completed && run.unresolved_question ? <p>{run.unresolved_question}</p> : null}
    ...
    <details key={`${run.run_id}:${run.status}`} className={styles.disclosure}><summary>How this was checked{steps.length > 0 && ` · ${completedSteps} of ${steps.length} recorded checks completed`}</summary><div className={styles.detailContent}>
